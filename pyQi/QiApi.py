...
class QiAPI(QiAuthentication):
    ...
    def lookup_table_id(self, table: str):
        if self.types_data is None:
            self.types_data = self.get_types()
    ...
    def lookup_list(self, table: str, field_name: str):
        self.list_dict = dict()
    ...
    def lookup_lists(self, table: str):
        if self.types_data is None:
            self.types_data = self.get_types()
        for header in self.column_headers:
            if "list:" in header:
    ...
    def get_request(self, table: str, fields_to_search: list[str]|str|None = None, search_term: list[str]|str|None = None, print_response: bool = False, **kwargs):
        method = "get"
        ...
        self._call_url_iter(url, method=method, data=None)
        return self.json_data
    
    def put_request(self, data: str|dict, table: str, auto_approve: bool = False, print_response: bool = False):
        self.print_response = print_response
    ...
    def delete_request(self, table: str, id_to_delete: int|str, auto_approve: bool = False, print_response: bool = False):
        method = "delete"
        field_url = "id"
        self.print_response = print_response
        ...
        self._call_url(url, method=method, data=None)

    def _call_url(self, url: str, method: str, data: str|dict|None = None):
        logger.info(f'Calling to: {url}')
        try:
            if method == "get":
                self.response = requests.get(url,auth=self.auth)
            if method == "put":
                self.response = requests.put(url,auth=self.auth,data=data)
            if method == "post":
                self.response = requests.post(url,auth=self.auth,data=data)
            if method == "delete":
                self.response = requests.delete(url,auth=self.auth)
    ...
    def _call_url_iter(self, url: str, method: str, data: str|dict|None = None):
        try:
            root_url = url 
            self._call_url(url, method=method, data=data)
    ...
    def search_to_excel(self, output_file: str, table: str, fields_to_search: str, search_term: str|None = None, print_response: bool = False, **kwargs):
        ...
        self.search_to_records(table = table, fields_to_search = fields_to_search, search_term = search_term, print_response = print_response, **kwargs)
        df = pd.DataFrame([vars(s) for s in self.qi_records.records])
        df.to_excel(output_file, index = False)

    def search_to_csv(self, output_file: str, table: str, fields_to_search: str, search_term: str|None = None, print_response: bool = False, **kwargs):
        if pd is None:
            log_msg = "Pandas library is required for this function. Install with: pip install pandas"
            logger.error(log_msg)
            raise ImportError(log_msg)
    ...
    def search_to_json_df(self, output_file: str, table: str, fields_to_search: str, search_term: str|None = None, print_response: bool = False, **kwargs):
        if pd is None:
            log_msg = "Pandas library is required for this function. Install with: pip install pandas"
            logger.error(log_msg)
            raise ImportError(log_msg)
    ...
    def search_to_json(self, output_file: str, table: str, fields_to_search: str, search_term: str|None = None, print_response: bool = False, **kwargs):
        self.search_to_records(table = table, fields_to_search = fields_to_search, search_term = search_term, print_response = print_response, **kwargs)
        self.qi_records.json_to_file(output_file)

    def search_to_json_string(self, table: str, fields_to_search: str, search_term: str|None = None, print_response: bool = False, **kwargs) -> str:
        self.search_to_records(table = table, fields_to_search = fields_to_search, search_term = search_term, print_response = print_response, **kwargs)
        json_string = self.qi_records.json_tostring()
        return json_string

    def search_to_xml(self, output_file: str, table: str, fields_to_search: str, search_term: str|None = None, print_response: bool = False, **kwargs):
        if pd is None:
            log_msg = "Pandas library is required for this function. Install with: pip install pandas"
            logger.error(log_msg)
            raise ImportError(log_msg)
    ...
    def search_to_df(self, table: str, fields_to_search: str, search_term: str|None = None, print_response: bool = False, **kwargs):
        if pd is None:
            log_msg = "Pandas library is required for this function. Install with: pip install pandas"
            logger.error(log_msg)
            raise ImportError(log_msg)
    ...
    def search_to_dict(self, table: str, fields_to_search: str, search_term: str|None = None, print_response: bool = False, **kwargs):
        self.search_to_records(table = table, fields_to_search = fields_to_search, search_term = search_term, print_response = print_response, **kwargs)
        dict_list = [vars(s) for s in self.qi_records.records]
        return dict_list

    def search_to_records(self, table: str, fields_to_search: str, search_term: str|None = None, print_response: bool = False, **kwargs):
        json_data = self.get_request(table = table, fields_to_search = fields_to_search, search_term = search_term, print_response = print_response, **kwargs)
    ...
    def update_from_file(self, file: str, table: str, auto_approve: bool = False, lookup_field: str|None = None, print_response: bool = False):
        self._read_source(file)
        node_id = self.lookup_table_id(table)
        self.column_headers = list(self.df.columns.values)
        if any("relationship:" in x for x in self.column_headers):
            ...
            if id is None:
                if lookup_field is not None:
                    ...
                    if lookup_term is None:
                        ...
                        logger.error(log_msg)
                        raise ValueError(log_msg)
                    self.get_request(table = table, fields_to_search = lookup_field, search_term = str(lookup_term), print_response = print_response)
    ...
    def update_from_search(self, table: str, fields_to_update: dict, fields_to_search: str, search_term: str, auto_approve: bool = False, print_response: bool = False):
        if self.types_data is None:
            self.get_types()
        ...
        search = self.get_request(table = table, fields_to_search = fields_to_search, search_term = search_term, print_response = print_response, **return_fields)
    ...
    def delete_from_file(self, file: str, table: str, auto_approve: bool = False, lookup_field: str|None = None, print_response: bool = False):
        self._read_source(file)
        records = self.df.to_dict('records')
        for row in records:
            id = row.get('id')
            if id is None:
                if lookup_field is not None:
                    ...
                    self.get_request(table = table, fields_to_search = lookup_field, search_term = lookup_term, print_response = print_response)
                    j = json.loads(self.response_text)
                    id = j['records'][0]['id']
    ...
    def delete_from_search(self, table: str, fields_to_search: str, search_term: str, auto_approve: bool = False, print_response: bool = False):
        search = self.get_request(table = table, fields_to_search = fields_to_search, search_term = search_term, print_response = print_response)
        for hit in search.get('records'):
